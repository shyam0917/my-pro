;define(['jquery', 'lib/CallGet', 'config!', 'videoplayer', 'videoswitcher', 'WebPlayer'],
    function ($, CallGet, config, videojs, videoswitcher, WebPlayer) {
    ...
    function Player(page, handlers) {
        ...
        this.showNextVideo          = function(courseID, lesson,  containerID, params, extraApiRequestParameters){
            ...
            this.loadSource(courseID, lesson, params, extraApiRequestParameters, captionType).done(function () {
                if(that.getLessonType()=="html"){
                    ...
                    window.scrollTo(0,50)//50 is a very good number
                    ...
                    htmlPlayer = new WebPlayer("#"+containerID,that.lessonGUID,that.localData.data.response.link[0].file);
        ...
        /*
        I use these variables to store value of variables which were used in previous function call.
        I reuse previous value in case when this function is called but parameter is not passed.
        I use it to reload the video of the same lesson but with new language when user chooses different language of the content.
         */
        /*
        The additionalCaptionsConfig is used to set up specific language/captions for certain pages
        */
        var courseIDLast = null
        var lessonLast = null
        var containerIDLast = null
        var paramsLast = null
        var extraApiRequestParametersLast = null
        var altCaptionsConfigLast = null
        this.show                   =   function(courseID, lesson,  containerID, params, extraApiRequestParameters, altCaptionConfig = false) {

            if(typeof courseID == 'undefined' && courseIDLast) courseID = courseIDLast
            if(typeof lesson == 'undefined' && lessonLast) lesson = lessonLast
            if(typeof containerID == 'undefined' && containerIDLast) containerID = containerIDLast
            if(typeof params == 'undefined' && paramsLast) params = paramsLast
            if(typeof extraApiRequestParameters == 'undefined' && extraApiRequestParametersLast) extraApiRequestParameters = extraApiRequestParametersLast
            if(typeof altCaptionConfig == 'undefined' && altCaptionsConfigLast) altCaptionConfig = altCaptionsConfigLast

            if(typeof courseID !== 'undefined') courseIDLast = courseID
            if(typeof lesson !== 'undefined') lessonLast = lesson
            if(typeof containerID !== 'undefined') containerIDLast = containerID
            if(typeof params !== 'undefined') paramsLast = params
            if(typeof extraApiRequestParameters !== 'undefined') extraApiRequestParametersLast = extraApiRequestParameters
            if(typeof altCaptionConfig !== 'undefined') altCaptionsConfigLast = altCaptionConfig
            ...
            var isCurrentLessonWatched  = params.isWatched ? params.isWatched : false;
            ...
            this.loadSource(courseID, lesson, params, extraApiRequestParameters, captionType).done(function () {
                ...
                if(that.getLessonType()=="html"){
                    ...
                    //override to local source (can be removed after code has been live a couple weeks.)
                    //designer, image, pdf, quiz, chat testing links
                    //that.localData.data.response.link[0].file="https://"+location.host+"/lessontemplates/text_question_text_multi_answer/en/v1/index--v"+Date.now()+".html"
                    //that.localData.data.response.link[0].file="https://"+location.host+"/DESEXP1-lkfhl/index--v"+Date.now()+".html"
                    //that.localData.data.response.link[0].file="https://"+location.host+"/DESEXP1-f8eux/index.html"
                    //that.localData.data.response.link[0].file="https://"+location.host+"/DESEXP1-c1a0m/index--v"+Date.now()+".html"
                    //that.localData.data.response.link[0].file="https://"+location.host+"/DESEXP1-pan5e/index--v"+Date.now()+".html"

                    htmlPlayer = new WebPlayer("#"+containerID,that.lessonGUID,that.localData.data.response.link[0].file);
                    //htmlPlayer.addPlayer(that.lessonGUID,"https://"+location.host+"/lessontemplates/text_question_text_multi_answer/en/v1/index--v"+Date.now()+".html")
                    ...
                    function() {
                        ...
                        videoPlayer.on("loadedmetadata", function(){
                            ...
                            if(captionType == 'alternative' && typeof params['tracks'] == 'object' && typeof params['tracks'][0] != 'undefined' && !params['disableCaptions']) {
                                ...
                                var siteLang  = page.getLanguage(),
                                    videoLang = params.sources[0].label;

                                var captionsArray = [];

                                var captionLang = localStorage.getItem('captionLang');

                                // For first we should get user selected lang
                                if (captionLang && captionLang !== 'none' && typeof that.localData.vttLang[captionLang] !== 'undefined')
                                    captionsArray = that.localData.vttLang[captionLang].data;
                                else if (that.localData.vttLang[videoLang])
                        ...
                        if( (fastForwardDisabled == true &&  transcriptAlt == false) && isCurrentLessonWatched == false) {

                            //allows users to seek backwards but not forwards.
                            var currentTime = 0;
                            var playerPaused = 0;
                            var alertShowedFlag = false;

                            videoPlayer.on("seeking", function(event) {
                                let currentCacheTime = videoPlayer.getCache().currentTime;
                                if(alertShowedFlag === false) {
                                    if (currentCacheTime < videoPlayer.currentTime()) {
                                        videoPlayer.currentTime(currentCacheTime);
                            ...
                            function showErrorWin(errorMsg) {
                                if(alertShowedFlag === false){
                                    videoPlayer.BlockSeekBarMouseMove = true;
                                    alertShowedFlag = true;
                                    page.showAlert(errorMsg, true).done( function (modalWindow) {
                                        modalWindow.bindOnClose(function () {
                                            $('body').removeAttr( 'style' );
                                            videoPlayer.BlockSeekBarMouseMove = false;
                                            alertShowedFlag = false;
                                            playerPaused = false;
                                            videoPlayer.play();
                                        });
                                    });
        ...
        this.loadSource                 = function (courseID, lesson, params, extraApiRequestParameters, captionType, altCaptionConfig = false) {
            ...
            var APIurl          = page.config.APIUrl;

            if(typeof params.lang == 'undefined' || !params.lang){ params.lang = page.getLanguage(); }

            // Looking for alternative video captions/languages Configuration
            if( typeof altCaptionConfig != 'undefined' && altCaptionConfig != false ){ params.lang = altCaptionConfig.lang; }
            ...
            $.getJSON(APIresourceURL, function(data) {
                ...
                if (typeof data.response.captions == "object" && !params['disableCaptions']) {
                    ...
                    let availableLanguages = page.getLanguages();

                    params['tracks'] = [];
                    $.each(data.response.captions, function (k, v) {

                        if(!~availableLanguages.indexOf(v.label)){
                            return;
                        }
                ...
                API resonse contains object `langs`. This object tells me how many languages the content of the lesson has. Meaning if the lesson hav VO in English and Arabic this object will containt two elements: en, ar. On the other hand if lesson contains VO only in one language this object will contain only that language.
                Here I detect if there are 2 or more languages then I show the "change language" button in the player.

                 */
                if (typeof data.response.langs == "object") {
                    params['contentAvailableLanguages'] = data.response.langs;
                    if(params['contentAvailableLanguages'].length > 1){
                        $(".video-player-settings").removeClass('hidden');
                        $("#switchContentLanguageList button").remove();
                        $.each(params['contentAvailableLanguages'], function(k, lang){
                            var button = $("<button></button>").html(lang+(data.response.link[0].label == lang? ' <i class="fa fa-check"></i>':''))
                            button.on('click', function(){
                                localStorage.setItem('lessonContentLanguage', lang);
                                params.lang = lang
                                // once the language is selected I reload video lesson with new language in params
                                that.show(undefined, undefined, undefined, params);
                                // that.loadSource(courseID, lesson, params, extraApiRequestParameters, captionType)
                            })
                            $("#switchContentLanguageList").append(button)
                        })
                    }else{
                        $(".video-player-settings").addClass('hidden');
                    }
                }
        ...
        this.handleResize       = function(e){
            //currently, function not used. generally we want css to handle, not a resize event
        ...
        //seems this method is not called. there's another in courses.js
        ...
        this.sizeForHtmlPlayer = function(){
            // sadly we have to undo the hard style that's being done in js to get a propper size.
            // this shouldn't happen here since it shouldn't mutate on getting size, but we had to
            // because of bad js that forces that inline style
            $('#courseVideoEmbed').attr('style', '');
        ...
        $(window).resize(this.handleResizeRef = e => {this.handleResize(e)});

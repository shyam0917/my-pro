;define(['jquery', 'jquery.mousewheel'], function ($) {
    ...
    function ModalWindow(options) {
        ...
        /*
         * If modal was cloned, component save link to parent node.
         *  parent node ID will by moved to data attribute for prevent duplicate node with one ID.
         *  After close and remove cloned modal, ID attribute will be return to parent node.
         */
        let parentNode              = null;
        ...
        this.show                   = function () {
            ...
            //if modal needs to be cloned (second time becomes alert, add here)
            var toClone = [
                "#invitationWindow",
                "#alertWindow",
                "#addedCourseModal",
            ...
            if (toClone.indexOf(modalID) > -1) {
                modalNode           = $(modalID).clone(true);
                parentNode          = $(modalID).data('id', modalNode.attr('id')).removeAttr('id');
            } else {
                modalNode =  $(modalID);
            }
            ...
            if($("#mContainer input:visible").length)
                $("#mContainer input:visible").eq(0).focus();
            else
                $("#mContainer button:visible").eq(0).focus();
        ...
        this.onKeyUp             = function (event) {
            ...
            if(event.which === 27 && options.isBlocking !== true) {
        ...
        this.close                  = function() {
            ...
            // Remove window container If special window
            modalContainer.find(modalID).remove();

            if(parentNode) {
                let id = parentNode.data('id');
                parentNode.attr('id', id);
            }
